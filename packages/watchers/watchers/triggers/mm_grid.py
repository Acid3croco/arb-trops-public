...
from watchers.sentinels.atr_sentinel import AtrSentinel
from watchers.trigger_base import TriggerBase, TriggerClient
from arb_defines.defines import LONG, SHORT, START, STOP, CANCEL, PING, PONG, UPDATE
from redis_manager.redis_events import OrderBookEvent, SentinelEvent, TradeEvent, TradeExecEvent, TriggerEvent
from arb_defines.arb_dataclasses import Candle, Instrument, Order, SentinelPayload, TriggerPayload, Trade
...
class MMGrid(TriggerBase):
    ...
    def subscribe_to_events(self):
        ...
        # self.redis_manager.subscribe_event(SentinelEvent(self.instruments, AtrSentinel.sentinel_name),
        #                                    self.on_atr_update)

    def on_atr_update(self, payload: SentinelPayload):
        self.logger.info(payload)
        if payload.action == UPDATE:
            values = payload.data.get('values')
            if values:
                self.config.atr_val = float(values[-1])

        self.logger.info(f'ATR UPDATED: {self.config.atr_val}')
        self.update_state()
    ...
    def _get_order_price(self, mult, side):
        ...
        if side == BUY:
            ...
            price = min(
                price, self.instrument.position.price
                if self.instrument.position.price != 0 else price)
            step = self.atr * (
            ...
            if mult == 0 and self.instrument.position.qty != 0:
                step = self.atr * (
                    (self.config.p_mult + self.config.buy_price_skew) * 1)
            ...
            if mult == 0 and self.instrument.position.qty != 0:
                step = self.atr * (
                    (self.config.p_mult + self.config.sell_price_skew) * 1)
            return (price + step) * (1 + self.config._p_shift), step

    def get_step(self, space, a):
        space = space * a
        if self.config.p_method == 'pow':
            return space**self.config.p_mult
        return space * self.config.p_mult
    ...
    def create_orders(self):
        ...
        for a in range(self.config.nb_orders):
            ...
            if self.instrument.position and self.instrument.position.price != 0 and self.instrument.position.qty < 0:
                ...
                price_check = not nearest_sell or (
                    price <= nearest_sell - step
                    or price >= furthest_sell + step)
            self.logger.debug(
